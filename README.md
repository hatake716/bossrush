...
戦闘32曲・ショップ・エンディングの**計34曲を、ZUN進行「♭VI → ♭VII → i → i」＋ピアノ＋ロック調**へ変更しました。戦闘は192〜232 BPM、1周28.24〜32.31秒。ピアノの主旋律・分散和音に、左右のエレキギター・ベース・ドラムを重ねます。トールは雷鎚の連打、海の神々は波のような分散和音、ヘルは高低の交替、ロキは3拍子、オーディンは二羽の鴉を表す応答を持ちます。曲の特徴は神話図鑑にも表示します。[全32曲の編曲意図とZUN進行の出典](docs/MUSIC.md)を参照してください。
...
- `BattleScore.kt`：全32体の主題・応答・神話モチーフ、ZUN進行の和音定義。
...
- `tools/music/`：全34曲の作曲データ・MIDI・音声生成スクリプト。
...
BGM変更の最新検証は[1.0.12の記録](docs/MUSIC_VALIDATION.md)、以前のゲーム機能と実操作・シミュレーション・テスト用状態の区別は[ゲーム機能の検証記録](docs/VALIDATION.md)に記載しています。ゲームバランスの最終的な手触りや音楽の聴感は、実機でのプレイによる継続調整の対象です。
