...
ハードモードは**ボスの攻撃力3倍・獲得スコア3倍**。通常攻撃・必殺技・突進・吹き飛ばしのダメージに共通で適用します。ボスのHP・技のタイミング・成長・所持金の倍率は通常モードと同じです。ハイスコアにはモードも表示します。[解放条件・保存仕様・検証記録](docs/HARD_MODE.md)。

## ハイスコア TOP 10

1.0.19から、スタートメニューの「ハイスコア」で過去の冒険の上位10件を閲覧できます。ゲームオーバーまたはゲームクリア時の合計スコアを記録し、スコアの高い順、同点なら新しい記録を上に並べます。職業・モード・討伐数・結果・記録日時も表示します。新しい冒険を始めても履歴は残り、アプリ終了後も保持されます。保存はこの端末内で行います。

旧版で保存していた最高スコアは「以前の最高記録」として引き継ぎます。職業・日時など、旧版に保存されていない情報は不明と表示します。旧版は最高スコア1件のみの保存だったため、それ以外の過去のプレイは復元できません。[保存仕様と検証記録](docs/HIGH_SCORES.md)。
...
剣・ナイフ・炎・氷は連続使用で3段コンボになります。技ボタンの段数表示は次に出す段。炎と氷は別々に段が進み、Lv.16の3段目は15発相当の魔法攻撃へ成長します。巨人とはにわは拳を伸ばして殴ります。[詳しい成長と操作](docs/PLAYER_COMBOS_1.0.23.md)。

## 技の成長と30秒基準

全技はLv.1開始、最大Lv.16。撃破ごとに1つだけレベルを上げられます。

強化画面を開くたびに、コントローラーのカーソルは必ず一番左の技から始まります。左端がLv.16でも初期位置は同じで、決定しても強化はされません。カーソルを合わせただけでは技を選択したことになりません。
...
HP1/2で初回の必殺技を発動した後は、通常技を1〜3回挟み、必殺技をランダムに再使用します。カットインは各戦闘の初回だけです。画面のどこかをタップするか、コントローラーのボタン（十字キー・左右トリガーを含む）を新しく押すまで表示を保ちます。待機中は戦闘時間・ダメージ・技の待機時間を止めます。押し続けていた攻撃やスティック移動では閉じず、閉じる入力も攻撃へ持ち越しません。2回目以降は戦闘と移動を止めず、予兆から発動します。通常技の選択も覚醒後はランダムになります。必殺技には各ボス専用の鮮やかな2色を使い、光柱・光線・属性の破片を追加しました。HP1/4以下では、各通常技の段や必殺技へ別の固有技の一撃を重ねる「二重詠唱」に移ります。基準となる発動間隔・連撃の間隔・予兆時間を従来の半分にし、4職業の移動速度を1.2倍にしました。共通の安全地点への移動に足りない場合は予兆を延ばします。カットインは世界樹、石造の神殿、組紐、神ごとの紋章を描いたドット絵で、攻撃の主色と連動します。[全32体の通常攻撃・難易度・覚醒後の仕組み](docs/COMBAT.md)を参照してください。
...
**1.0.23：主人公の攻撃を強化。** 剣・ナイフ・炎・氷に3段コンボ、成長する魔法弾幕、巨人とはにわの拳のアニメーションを追加しました。[操作・演出・威力・検証仕様](docs/PLAYER_COMBOS_1.0.23.md)、[Google Play用データ](play/README.md)。
...
- `PlayerCombat.kt` / `PlayerAttackArt.kt`：3段コンボ、魔法の弾数と威力配分、色付きの斬撃・火球・氷弾、召喚獣のパンチ。
