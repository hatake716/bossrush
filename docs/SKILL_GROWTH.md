...
| 召喚士・白ウサギの回復 | 8 → 19.2 → 32 | 80 → 102.4 → 128 |
...
1.0.16で、はにわは方向を問わず被ダメージ1回を完全に肩代わりし、その場で消滅する効果へ変更しました。被弾しなくても5秒で消滅します。身代わり回数と寿命は全レベルで固定し、近接攻撃の威力・範囲・待機時間・演出は従来どおり成長します。近接攻撃で寿命は延長されません。白ウサギの回復量は全レベルで従来の半分となり、回復間隔・範囲・召喚時間は変わりません。[仕様と検証](SUMMON_BALANCE.md)。
