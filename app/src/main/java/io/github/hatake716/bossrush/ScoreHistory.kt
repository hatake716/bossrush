...
internal object ScoreHistoryCodec {
    fun encode(records: List<ScoreRecord>): String = JSONObject().put("version",1)
        .put("records",JSONArray().apply {
            HighScores.ranked(records).forEach { r ->
                put(JSONObject().put("score",r.score).put("outcome",r.outcome.name)
            ...
            HighScores.ranked((0 until rows.length()).mapNotNull { i ->
                ...
                try {
                    ...
                    ScoreRecord(score.toInt(),outcome,
                        ...
                        if(outcome==ScoreOutcome.LEGACY) null else row.getLong("finishedAt")).takeIf { it.valid }
...
internal class ScoreHistory(private val prefs: SharedPreferences) {
    ...
    fun finish(run: Run,clear: Boolean,at: Long=System.currentTimeMillis()): List<ScoreRecord> {
        val record=ScoreRecord(run.score,if(clear) ScoreOutcome.CLEAR else ScoreOutcome.GAMEOVER,run.job,run.kills,at)
